...
- **Compare mode plane changes (coronal/sagittal)**: Vulkan textures were allocated at axial dimensions (512×512), so coronal/sagittal slices (512×326) caused data corruption when uploaded. Fixed by allocating textures at `max(imageSize)` dimensions, zero-padding staging buffer before upload, and scaling UV coordinates via new `gpuTextureWidth/Height` fields in `CalcDimsStruct` so only the valid data region is sampled. Panels 3/4 (pre-permuted sagittal/coronal) continue to skip plane changes as before.
- **SUV Quantitative Metrics autofill**: The "SUV Quantitative Metrics & References" dropdown now auto-selects the most relevant clinical criterion based on computed SUVmax vs. reference organs (blood pool, liver, parotid). Supports PROMISE scoring tiers, LBR ratio, and context-dependent rules (18F-PSMA bone lesions, UBU). Manual user selections are preserved.
- **Bone subsegmentation diagnostics**: Added `[BONE-MORPH]`/`[BONE-FAST]` debug logging to identify whether the proper morphological erosion or the simplified fast fallback is used. Exception backtraces are now printed on fallback to aid debugging.
- **Paint position offset (DPI scaling mismatch)**: GLFW cursor coordinates are in window-space but `CalcDimsStruct` used framebuffer-space dimensions (1.5× larger on DPI-scaled displays). Fixed by separating window size (for quad vertices/coordinate mapping) from framebuffer size (for Vulkan swapchain). `ResizeWindowEvent` now carries both sizes.
- **Bone subsegmentation quality**: On-the-fly bone surface/marrow computation used a simplified distance-based approach. Now calls `BoneSubsegmentation.generate_bone_subsegments` which uses proper 3D morphological erosion (6-connected) for cortical shell extraction and GPU-accelerated distance kernels for marrow selection.
- **Vulkan ERROR_OUT_OF_DATE_KHR crash**: `render_frame!` crashed when `vkAcquireNextImageKHR` returned `ERROR_OUT_OF_DATE_KHR` because the error check `isa(result, Vulkan.VulkanError)` never matched (result is a `ResultTypes.Result` wrapper). Fixed with try/catch around `unwrap()`. Consumer loop now automatically recreates swapchain on failure.
...
- **New lesion creation jumps to volume center**: The `on(active_lesion_id)` callback always sent `SyncLesionEvent`, even for newly created lesions with no voxels. Added `_skip_sync_on_lesion_change` flag that `btn_new_lesion` sets before changing the active ID, preventing the centroid-jump for empty lesions.
- **Bone type change doesn't trigger bone subsegmentation**: The bone type button handler parsed the active lesion ID by splitting on `" - "`, but new lesion names use the `"N: Name"` format. Replaced with `parse_lesion_id()` which handles both formats.
