...
# Helper: capture screenshot via consumer thread (synchronized)
function capture_synced(ch, path)
    done = Channel{Bool}(1)
    put!(ch, ScreenshotEvent(path=path, done_channel=done))
    take!(done)
end
...
# ========================================================
println("\n--- TEST 1: Spleen slice 31 + mask overlay ---")
put!(ch, 30); sleep(3)
...
# ========================================================
# TEST 4: Different slices (anatomy changes)
# ========================================================
println("\n--- TEST 4: Upper + Lower slices ---")
put!(ch, 15); sleep(2)
