...
float $(x[3])$(x[1])getColorForMultiColor(float innertexelRes) {
 ...
 float texelRes=  clamp(innertexelRes, $(x[1])minValue,$(x[1])maxValue );
        ...
        return  clamp(colorFloats[indexx +1]*normalizedColorPercent  +  colorFloats[indexx]*(1- normalizedColorPercent) ,0.0,0.3 );//so we get color from current section and closer we are to the end of this section the bigger influence of this color, the closer we are to the begining the bigger the influence of previous section color
}
