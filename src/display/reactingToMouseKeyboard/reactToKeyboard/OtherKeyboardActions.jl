...
function processKeysInfo(numbb::Identity{Int64}, stateObject::StateDataFields, keyInfo::KeyboardStruct, toBeSavedForBack::Bool=true)
    ...
    updateImagesDisplayed(stateObject.currentlyDispDat, stateObject.mainForDisplayObjects, stateObject.textDispObj, stateObject.calcDimsStruct, valueForMasToSett, stateObject.crosshairFields, stateObject.mainRectFields)
    # for undoing action
    # if(toBeSavedForBack)
