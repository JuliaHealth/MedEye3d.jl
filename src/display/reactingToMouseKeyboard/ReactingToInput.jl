...
function setUpWordsDisplay(textDispObject:: ForWordsDispStruct,mainState::StateDataFields)
    ...
    ,mainState.mainForDisplayObjects.vertex_shader
    ,textDispObject.vbo_words
    ,mainState.calcDimsStruct)

    texId =  createTexture(UInt8,mainState.calcDimsStruct.textTexturewidthh
                            ,mainState.calcDimsStruct.textTextureheightt
    ...
    reactivateMainObj(mainState.mainForDisplayObjects.shader_program
    , mainState.mainForDisplayObjects.vbo
    ,mainState.calcDimsStruct    )
...
function setUpvalueForMasToSet(valueForMasToSett::valueForMasToSetStruct ,mainState::StateDataFields)
    ...
    updateImagesDisplayed(mainState.currentlyDispDat
    , mainState.mainForDisplayObjects
    , mainState.textDispObj
    , mainState.calcDimsStruct ,valueForMasToSett )
