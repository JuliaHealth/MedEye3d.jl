...
"""
function loadFromHdf5Prim(fid, patienGroupName::String, addTextSpecs::Vector{TextureSpec}, listOfColorUsed, fractionOfMainIm::Float32=Float32(0.8))
  ...
  for maskName in imagesMasks
    ...
    textureSpec = if (maskName in addTextureNames)
      ...
      getDefaultTextureSpec(dataTypeStr, maskName, index, listOfColorUsed, typp, min, max, voxels)
    end
  ...
  datToScrollDimsB = ForDisplayStructs.DataToScrollDims(
    imageSize=imageSize, voxelSize=spacing, dimensionToScroll=3)
  ...
  coordinateDisplay(textureSpecifications, fractionOfMainIm, datToScrollDimsB, 1000)
...
"""
function getDefaultTextureSpec(dataTypeStr::String, maskName::String, index::Int, listOfColorUsed, typp, min, max, voxels)::TextureSpec
  ...
  elseif (dataTypeStr == "boolLabel")
    return TextureSpec{typp}(
      name=maskName,
      color=getSomeColor(listOfColorUsed), minAndMaxValue=typp.([min, max])
    )
  ...
  elseif (dataTypeStr == "multiDiscreteLabel")
    ...
    return TextureSpec{typp}(
      ...
      colorSet=colorSet, minAndMaxValue=typp.([min, max])
    )
  ...
  elseif (dataTypeStr == "contLabel")
    return TextureSpec{typp}(
      name=maskName,
      isContinuusMask=true,
      colorSet=[getSomeColor(listOfColorUsed), getSomeColor(listOfColorUsed)], minAndMaxValue=typp.([min, max])
    )
...
function calculateAndDisplay(preparedDict, mainScrollDat::FullScrollableDat, conf::ConfigurtationStruct, numberToLookFor, goldArr, algoOutputGPU)
...
function refresh(actor)
  ...
  singleSlDat = actor.actor.onScrollData.dataToScroll |>
                (scrDat) -> map(threeDimDat -> threeToTwoDimm(threeDimDat.type, Int64(current), actor.actor.onScrollData.dimensionToScroll, threeDimDat), scrDat) |>
                            (twoDimList) -> SingleSliceDat(listOfDataAndImageNames=twoDimList, sliceNumber=current, textToDisp=DisplayWords.getTextForCurrentSlice(actor.actor.onScrollData, Int32(current)))
