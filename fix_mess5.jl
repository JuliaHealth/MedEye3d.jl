...
for i in 1:length(lines)
    ...
    if skipping && occursin("_lmw_observables[:loading_overlay_txt] = loading_txt", line)
        skipping = false
        continue
    end
    if skipping
    ...
    if !skipping && length(new_lines) > 0 && occursin("_lmw_observables[:loading_overlay_txt]", lines[max(1, i-1)]) && strip(line) == ""
        continue
    end
