...
def extract_bone_fragments(lesion_path, bone_path, out_surface_path, out_marrow_path):
    ...
    if target_bone_label > 0:
        ...
        # Check if skellytour cortical (2) and marrow (1) labels exist, or fallback to morphological erosion
        if np.any(crop_skelly_mask == 2) or np.any(crop_skelly_mask == 1):
            crop_cortical = (crop_skelly_mask == 2)
            crop_marrow = (crop_skelly_mask == 1)
        else:
            eroded = ndimage.binary_erosion(target_bone_mask, iterations=2)
            crop_cortical = target_bone_mask & ~eroded
            crop_marrow = eroded
