...
adapted from https://discourse.julialang.org/t/3d-medical-app-stencil/64019
    ...
    @parallel_indices (ix,iy,iz) function computeMeanStd!(Mean::Data.Array, Std::Data.Array, In::Data.Array,aa::CuDeviceArray{Float64, 3, 1},bb::CuDeviceArray{Float64, 3, 1})
        ...
        if (ix<=size(Mean,1) && iy<=size(Mean,2) && iz<=size(Mean,3))
            ...
            Mean[ix,iy,iz] = (In[CartesianIndex(ixi-1,iyi  ,izi ) ] +
                              In[CartesianIndex(ixi-1,iyi  ,izi ) ] + In[ixi+1,iyi  ,izi  ] +
                              In[CartesianIndex(ixi  ,iyi-1,izi ) ] + In[ixi  ,iyi+1,izi  ] +
    ...
    @views function img_process()
        ...
        # Calculation
        @parallel computeMeanStd!(MeanB, Std, In, x_d, x_y)
